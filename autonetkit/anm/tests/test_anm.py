...
def test():
    ...
    assert ([i.description for i in sorted(node_interfaces)]
            == ['loopback', 'eth0'])
    ...
    assert(sorted(eth0.neighbors()) == [
           g_phy.node("r2").interface(1), g_phy.node("r3").interface(1)])
    ...
    # TODO: need to document API to check exists/doesn't exist for nodes,
    # edges, interfaces, graphs,....
